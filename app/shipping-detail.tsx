      ...
      bottomProps={{
        ...
        saveProps: {
          ...
          onPress: async () => {
            if (formState?.STOCKDETAIL_BARCODE_ID)
              axiosPost({
                path: `/Stock/CreateStockDetailForInstruction?barcode=${formState.STOCKDETAIL_BARCODE_ID}&quantity=${formState.STOCKDETAIL_QUANTITY}&stockMasterId=${data.masterId}&instructionDetailId=${formState.STOCKDETAIL_REF_ID}&piece=${formState.STOCKDETAIL_PIECE}`,
                success: data => {
        ...
        deleteProps: {
          ...
          onPress: () => {
            if (formState?.ID) {
              axiosDelete({
                path: `/Stock/DeleteStockDetail/${formState.ID}?link=SalesInstructionDetail`,
                success: () => setFormState(undefined)
              })
            }
          }
        },
        ...
        historyProps: {
          onPress: () => {
            router.push({ pathname: '/barcode-history', params: { masterId: data.masterId, type: 'stock' } })
          }
        }
      }}
    >
      <View style={{ display: 'flex', gap: 10, paddingHorizontal: 16 }}>
        <InputBarcode
          ...
          onSearchButton={() => {
            if (formState?.STOCKDETAIL_BARCODE_ID)
              axiosGet({
                path: `/Stock/GetStockDetailForInstruction?barcode=${formState?.STOCKDETAIL_BARCODE_ID}&stockMasterId=${data.masterId}`,
                success: (data: any) => {
                  setFormState(data)
                },
                error: () => {
        ...
        <View style={{ display: 'flex', flexDirection: 'row', width: '100%', gap: 10 }}>
          <MyNumberInput
            label={'Talimat Miktarı'}
            readOnly
            returnKeyType='next'
            containerStyle={{ flex: 1 }}
            value={formState?.STOCKDETAIL_INSTRUCTIONQUANTITY}
          />
          <MyNumberInput
            label={'Stok Miktarı'}
            readOnly
        ...
        <View style={{ display: 'flex', flexDirection: 'row', width: '100%', gap: 10 }}>
          ...
          <MyNumberInput
            ...
            returnKeyType='next'
            containerStyle={{ flex: 1 }}
            value={formState?.STOCKDETAIL_DEBITQUANTITY}
          />
        </View>
      </View>
    </Layout>
