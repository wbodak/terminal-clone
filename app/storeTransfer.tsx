...
const StoreTransfer = () => {
  ...
  const getStoreTransfers = () => {
    if (formState.sourceStore || formState.targetStore) {
      ...
      axiosGet({
        path: `/Stock/GetStockMasterListByDate?sourceStore=${formState.sourceStore || null}&targetStore=${formState.targetStore || null}&startDate=${today}&endDate=${today}&typeCode=57`,
        success: data => {
          setStockList(data)
        }
      })
    ...
    >
      ...
      <MyDataGrid
        ...
        columns={[
          ...
          { dataField: 'STOCKMASTER_TARGETSTORENAME', caption: 'Hedef Depo', dataType: 'string' },
