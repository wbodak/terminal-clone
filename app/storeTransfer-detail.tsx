...
const StoreTransferDetail = () => {
  ...
  return (
    <Layout
      headerTitle='Depolar Arası Transfer Detayı'
      showSubTitle={false}
      bottomProps={{
        ...
        saveProps: {
          ...
          onPress: async () => {
            if (formState?.STOCKDETAIL_BARCODE_ID)
              axiosPost({
                path: `/Stock/CreateStockDetailForStoreTransfer?barcode=${formState?.STOCKDETAIL_BARCODE_ID}&quantity=${formState.STOCKDETAIL_QUANTITY}&stockMasterId=${data.masterId}&piece=${formState.STOCKDETAIL_PIECE}`,
                success: data => {
    ...
    >
      <View style={{ display: 'flex', gap: 10, paddingHorizontal: 16 }}>
        ...
        {/* <BarcodeHistory masterId={Number(data.masterId)} /> */}
      </View>
    </Layout>
  )
}

export default StoreTransferDetail
