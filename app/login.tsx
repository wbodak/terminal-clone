...
import { getDataFromStorage, saveDataToStorage, removeDataFromStorage } from '@/utils/asyncStore'
...
const Login = () => {
  ...
  const selectedCorp = selectionData.find(corp => corp.corporationId === corpId)
        ...
        data.forEach((userCorporation: V_SetUserCorporation) => {
          if (corporations.length > 0) {
            corporations.forEach(corporation => {
              if (corporation.corporationId === userCorporation.SETUSERCORPORATION_CORP_ID) {
          ...
          setCorporationDtos(corporations.map(corporation => corporation.data))
        })
      }
    })
  }
  ...
  return (
    <>
      ...
      >
        ...
        />
        <View>
          <MyText style={{ paddingLeft: 33, fontSize: 24, fontWeight: 900 }}>Giriş yapın</MyText>
          <View
            style={{
          ...
          >
            ...
            {!hasApiUrl ? (
              <>
                <MyText style={{ marginBottom: 42, fontSize: 14, fontWeight: 300 }}>
                  Bir kereliğine girilecek olan sunucu adresini giriniz.
                </MyText>
                <View style={{ marginVertical: 'auto', display: 'flex', gap: 12 }}>
                  <MyInput
                    theme={'light'}
                  ...
                  >
                    <MyText style={{ fontWeight: 800, fontSize: 14 }}>Kaydet</MyText>
                  </MyButton>
                </View>
              </>
            ) : (
              <>
                <MyText style={{ marginBottom: 42, fontSize: 16, fontWeight: 300 }}>
                  Yöneticiniz tarafından size verilen giriş bilgilerini kullanarak sisteme giriş yapabilirsiniz.
                </MyText>
                    ...
                    onPress={() => {
                      axiosPost({
                        ...
                        success: async (data: WebUserDto) => {
                          await saveDataToStorage('userToken', data.token || '')
                          await saveDataToStorage('userData', JSON.stringify(data))
                          router.replace('/home')
                        }
                      })
                    }}
                  >
                    <MyText style={{ fontWeight: 800, fontSize: 14 }}>Giriş Yap</MyText>
                  </MyButton>
                </View>
              </>
            )}
        ...
        <MyText style={{ position: 'absolute', bottom: 24, left: 0, right: 0, margin: 'auto', textAlign: 'center' }}>
