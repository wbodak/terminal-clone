        ...
        saveProps: {
          ...
          onPress: async () => {
            if (formState?.STOCKDETAIL_BARCODE_ID)
              axiosPost({
                path: `/Stock/CreateStockDetailForStoreTransfer?barcode=${
                  formState?.STOCKDETAIL_BARCODE_ID
                }
                &quantity=${formState.STOCKDETAIL_QUANTITY}&stockMasterId=${
                  data.masterId
                }
                &piece=${Number(
                  String(formState.STOCKDETAIL_PIECE).replace(/\./g, "")
                )}`,
                success: () => {
        ...
        <View style={styles.rowContainer}>
          ...
          {formState?.STOCKDETAIL_ISPROFILE && (
            <MyInput
              label={"Profil"}
              readOnly
              containerStyle={styles.widthThirtyFive}
              returnKeyType="next"
