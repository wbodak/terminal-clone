    ...
    <MyButton
      ...
      onPress={() => {
        showDialog({
          ...
          callback: () => {
            axiosDelete({
              path: `${deletePath}/${masterId}${queries}`,
              success: onScuccess,
            });
          },
        });
...
export const SelectButton = ({ data }: any) => {
  ...
  const setGridSelectedElement = useStore(
    (state) => state.setGridSelectedElement
  );
  ...
  return (
    ...
    >
      {gridSelectedElement[data.ID] ? (
        <Image
          source={icons["checkbox-check"]}
          style={{ width: 28, height: 28 }}
        />
      ) : (
        <Image
          source={icons["checkbox-uncheck"]}
          style={{ width: 28, height: 28 }}
        />
      )}
    </MyButton>
